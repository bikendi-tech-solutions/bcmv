...
<div class="card">
                <div class="card-body">
                  <h5 class="card-title">Pending Cable Subscriptions</h5>

                  <?php

if(!isset($_GET["trans_id"])){
    pagination_history_before("scable","false");
}
elseif(empty($_GET["trans_id"])){
  pagination_history_before("scable","false");
    ...
    if(is_numeric($_GET["trans_id"])){
        $id = $_GET["trans_id"];
        pagination_history_before("scable","false","AND id = '$id'");
    }
    else{
        $id = $_GET["trans_id"];
        pagination_history_before("scable","false","AND request_id = '$id'");
    }
}
...
foreach($transactions as $result){
    ...
    <button class='bg bg-danger' onclick='reversetransaction(\"scable\",\"".$result->id."\", \"".$result->user_id."\", \"".$result->amount."\");'> <i class=' fas fa-retweet  bg-danger text-white'></i></button>
    <button class='bg bg-success' onclick='approvetransaction(\"scable\",\"".$result->id."\", \"".$result->user_id."\", \"".$result->amount."\");'> <i class=' fas fa-check bg-success text-white'></i></button>
    </div>
    </td>
...
<label class="customcheckbox mb-3">
  ...
  <select onchange="openfunction('scable','CableTv')">
