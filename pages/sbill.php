...
<div class="card">
                <div class="card-body">
                  <h5 class="card-title">Successful Bill Payments</h5>

                  <?php

if(!isset($_GET["trans_id"])){
    pagination_history_before("sbill","true");
}
elseif(empty($_GET["trans_id"])){
  pagination_history_before("sbill","true");
    ...
    if(is_numeric($_GET["trans_id"])){
        $id = $_GET["trans_id"];
        pagination_history_before("sbill","true","AND id = '$id'");
    }
    else{
        $id = $_GET["trans_id"];
        pagination_history_before("sbill","true","AND request_id = '$id'");
    }
}
...
foreach($transactions as $result){
    ...
    <button class='' onclick='reversetransaction(\"sbill\",\"".$result->id."\", \"".$result->user_id."\", \"".$result->amount."\");'> <li class=' fas fa-retweet'></li></button>
    </div>
    </td>
...
<label class="customcheckbox mb-3">
  ...
  <select onchange="openfunction('sbill','Utility',false)">
